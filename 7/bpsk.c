...
int main(){
    int bits[] = {0,1,0,1,1,0,1,1};
    int count = 8; //количество битов
    ...
    filter(duration, I_upsampled, Q_upsampled, I_filtred, Q_filtred);

    sdvig(duration, I_filtred, Q_filtred);
    
    int16_t tx_buff[2*1920];

    to_buff(I_filtred, Q_filtred, tx_buff, sample);

    //выставляем timestamp
